...
impl MuteIndicator {
    ...
    pub fn apply_mute(&self, device: &impl ScarlettDevice) -> Result<()> {
        led::apply_mute_indicator_with_mode(
            device,
            &self.strategy,
            self.mute_color,
            self.render_mode,
    ...
    pub fn clear_mute(&self, device: &impl ScarlettDevice) -> Result<()> {
        led::clear_mute_indicator_with_mode(device, &self.strategy, self.render_mode)
    }

    /// Restore the display after this indicator owned it.
    pub fn restore(&self, device: &impl ScarlettDevice) -> Result<()> {
    ...
    /// Change display rendering without changing the mute state itself.
    pub fn set_render_mode(&mut self, mode: led::IndicatorRenderMode) {
    ...
    pub fn animate(&mut self, device: &impl ScarlettDevice, now: Instant) -> Result<()> {
        ...
        let Some(deadline) = self.next_blink else {
            self.next_blink = Some(now + Duration::from_millis(500));
            return self.apply_mute(device);
        };
        if now < deadline {
            return Ok(());
        }
        if self.blink_off {
            self.apply_mute(device)?;
        } else {
            led::blank_mute_indicator(device, &self.strategy)?;
        }
    ...
    pub fn render_mode(&self) -> led::IndicatorRenderMode {
        self.render_mode
    }
...
mod tests {
    ...
    #[test]
    fn solo_mute_blink_restores_white_on_unmute_and_stops_writing() {
        let mut dev = MockDevice::new();
        dev.info_mut().device_name = "Scarlett Solo 4th Gen-TEST".into();
        let mut indicator = make_indicator(true);
        indicator.set_render_mode(led::IndicatorRenderMode::NumbersBlink);
        let now = Instant::now();
        indicator.animate(&dev, now).unwrap();
        assert_eq!(
            dev.descriptors.borrow().get(&84),
            Some(&vec![led::solo::INPUT_2_LED])
        );
        indicator
            .animate(&dev, now + Duration::from_millis(499))
            .unwrap();
        assert_eq!(dev.notifies.borrow().len(), 2);
        indicator
            .animate(&dev, now + Duration::from_millis(500))
            .unwrap();
        assert_eq!(dev.descriptors.borrow().get(&80), Some(&vec![0; 4]));
        indicator.poll_and_apply(false, &dev);
        let (action, error) = indicator.poll_and_apply(false, &dev);
        assert_eq!(action, MonitorAction::ClearMute);
        assert!(error.is_none());
        assert_eq!(
            dev.descriptors.borrow().get(&80),
            Some(&led::solo::WHITE.to_le_bytes().to_vec())
        );
        let writes = dev.notifies.borrow().len();
        indicator
            .animate(&dev, now + Duration::from_secs(10))
            .unwrap();
        assert_eq!(dev.notifies.borrow().len(), writes);
