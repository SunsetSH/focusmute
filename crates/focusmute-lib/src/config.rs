...
pub struct IndicatorConfig {
    ...
    /// `auto` / `numbers`: steady number indication; `numbers_blink`: periodic blink.
    /// Legacy `halos_solid` is accepted but falls back to numbers.
...
pub struct SystemConfig {
    ...
    /// User interface language: `en` or `ru`.
...
/// Optional hardware-button integration. It is opt-in because Scarlett
/// firmware applies Direct before the application observes the event.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HardwareButtonConfig {
    #[serde(default)]
    pub direct_button_enabled: bool,
    #[serde(default = "default_direct_double_click_ms")]
    pub direct_double_click_ms: u64,
}

impl Default for HardwareButtonConfig {
    fn default() -> Self {
        Self {
            direct_button_enabled: false,
            direct_double_click_ms: default_direct_double_click_ms(),
        }
    }
}
...
fn default_direct_double_click_ms() -> u64 {
    350
}
...
pub const VALID_INDICATOR_MODES: &[&str] = &["auto", "numbers", "numbers_blink", "halos_solid"];
pub const VALID_LANGUAGES: &[&str] = &["en", "ru"];
...
pub struct Config {
    ...
    #[serde(default)]
    pub hardware_button: HardwareButtonConfig,
}
...
impl From<LegacyConfig> for Config {
    fn from(legacy: LegacyConfig) -> Self {
        Config {
            ...
            hardware_button: HardwareButtonConfig::default(),
        }
    }
}
