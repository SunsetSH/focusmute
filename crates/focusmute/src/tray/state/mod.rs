...
impl TrayState {
    ...
    pub fn init_without_device(config: Config) -> Self {
        ...
        let mut indicator = MuteIndicator::new(2, false, init_mute_color, noop_strategy);
        indicator.set_render_mode(led::IndicatorRenderMode::from_config(
            &config.indicator.mode,
        ));
        indicator.set_blink_while_muted(
    ...
    pub fn try_reconnect(&mut self) -> Option<focusmute_lib::device::PlatformDevice> {
        ...
        {
            // Always resolve again: a reconnect can be a different Scarlett or
            // a firmware update with another descriptor layout.
    ...
    pub fn restore_on_exit(&self, device: &impl ScarlettDevice) {
        if let Err(e) = self.indicator.restore(device) {
...
pub fn handle_menu_event(
    ...
    settings_toggle_callback: Option<crate::settings_dialog::SettingsToggleCallback>,
) -> (bool, bool) {
    if event.id() == menu.quit_item.id() {
        ...
        let input_count = state
            .ctx
            .as_ref()
            .and_then(|c| c.input_count())
            .unwrap_or(0);
        if let Some(new_config) = crate::settings_dialog::show_settings(
            &state.config,
            input_count,
            info,
            ...
            menu.sync_language(&state.config, device.is_some(), state.indicator.is_muted());
        ...
        // The modal lab may have restored white on close, including Cancel.
        // Reconcile Solo with the current mute indication before resuming.
        if let Some(dev) = device.as_ref()
            && focusmute_lib::direct::is_solo(dev)
        {
            let result = if state.indicator.is_muted() {
                state.indicator.apply_mute(dev)
            } else {
                state.indicator.clear_mute(dev)
            };
            if let Err(error) = result {
                log::warn!("[led] could not restore indication after settings: {error}");
            }
        }
