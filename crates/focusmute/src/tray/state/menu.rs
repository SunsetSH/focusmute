...
pub fn set_tray_mute_state(tray: &tray_icon::TrayIcon, muted: bool) {
    // Tooltip is intentionally compact; detailed localized state is in menu.
    if muted {
...
impl TrayMenu {
    ...
    pub fn set_reconnect_label(&self, connected: bool) {
        self.reconnect_item.set_text(crate::i18n::tr(
            &self.language,
            if connected { "resync" } else { "reconnect" },
        ));
    }

    /// Repaint all persistent tray labels after a language change.
    pub fn sync_language(&mut self, config: &Config, connected: bool, muted: bool) {
        self.language = config.system.language.clone();
        self.toggle_item.set_text(format!(
            "{}\t{}",
            crate::i18n::tr(&self.language, "toggle_mute"),
            config.keyboard.hotkey
        ));
        self.settings_item.set_text(format!(
            "{}...",
            crate::i18n::tr(&self.language, "settings")
        ));
        self.quit_item
            .set_text(crate::i18n::tr(&self.language, "quit"));
        self.set_reconnect_label(connected);
        self.status_item.set_text(if connected {
            crate::i18n::tr(&self.language, if muted { "muted" } else { "live" })
        } else {
            crate::i18n::tr(&self.language, "connect_device")
        });
    }
    ...
    pub fn set_device_connected(&self, connected: bool, tray: &tray_icon::TrayIcon) {
        self.status_item.set_text(if connected {
            crate::i18n::tr(&self.language, "live")
        } else {
            crate::i18n::tr(&self.language, "connect_device")
        });
        self.set_reconnect_label(connected);
...
pub fn build_tray_menu(config: &Config, initial_muted: bool) -> (Menu, TrayMenu) {
    let menu = Menu::new();
    let initial_status = if initial_muted {
        crate::i18n::tr(&config.system.language, "muted")
    } else {
        crate::i18n::tr(&config.system.language, "live")
    };
    let status_item = MenuItem::new(initial_status, false, None);
    let toggle_label = format!(
        "{}\t{}",
        crate::i18n::tr(&config.system.language, "toggle_mute"),
        config.keyboard.hotkey
    );
    ...
    let settings_item = MenuItem::new(
        format!(
            "{}...",
            crate::i18n::tr(&config.system.language, "settings")
        ),
        true,
        None,
    );
    let reconnect_item = MenuItem::new(
        crate::i18n::tr(&config.system.language, "reconnect"),
        true,
        None,
    );
    let quit_item = MenuItem::new(crate::i18n::tr(&config.system.language, "quit"), true, None);
...
pub fn apply_mute_ui(
    ...
    match action {
        ...
        MonitorAction::ClearMute => {
            ...
            menu.status_item
                .set_text(crate::i18n::tr(&state.config.system.language, "live"));
            if play_sound {
                sound::play_sound(
