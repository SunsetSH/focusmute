...
/// Classifies Direct *state changes* into a delayed single or a double gesture.
/// Scarlett applies Direct in firmware first, so the single action restores its
/// previous state before requesting Windows mute.
struct DirectGesture {
    pending: Option<(bool, Instant)>,
    ignore_until: Instant,
    expected_own_state: Option<(bool, Instant)>,
}

#[derive(Clone, Copy)]
enum DirectAction {
    Single { baseline: bool },
    Double { baseline: bool },
}
...
impl DirectGesture {
    fn new() -> Self {
        Self {
            ...
            expected_own_state: None,
        }
    }

    fn changed(&mut self, now: Instant, current: bool, window: Duration) -> Option<DirectAction> {
        if self
            .expected_own_state
            .is_some_and(|(expected, deadline)| now <= deadline && current == expected)
        {
            self.expected_own_state = None;
            return None;
        }
        if self
            .expected_own_state
        ...
        if let Some((baseline, _)) = self.pending.take() {
            return Some(DirectAction::Double { baseline });
        }
        self.pending = Some((!current, now + window));
        None
    }

    fn tick(&mut self, now: Instant) -> Option<DirectAction> {
        let (baseline, deadline) = self.pending?;
        if now < deadline {
            return None;
        }
        self.pending = None;
        Some(DirectAction::Single { baseline })
    }
    ...
    fn suppress_own_notifications(&mut self, now: Instant, expected_state: bool) {
        self.ignore_until = now + Duration::from_millis(200);
        self.expected_own_state = Some((expected_state, now + Duration::from_secs(2)));
    }
}

fn handle_direct_button<M: MuteMonitor>(
    device: &impl ScarlettDevice,
    monitor: Option<&M>,
    muted: bool,
    enabled: bool,
    window_ms: u64,
    gesture: &mut DirectGesture,
) {
    if !enabled || !focusmute_lib::direct::is_solo(device) {
        return;
    }
    let now = Instant::now();
    match device.wait_notify(0) {
        Ok(packet) => {
            let Some(mask) = focusmute_lib::direct::notification_mask(&packet) else {
                log::warn!(
                    "[direct] ignored short device notification ({} bytes)",
                    packet.len()
                );
                return;
            };
            if mask & focusmute_lib::direct::SOLO_VIDEO_CALL_NOTIFY_MASK != 0 {
                // A three-second hold is reserved by Solo for Combine Inputs.
                gesture.cancel();
                return;
            }
            if mask & focusmute_lib::direct::SOLO_DIRECT_NOTIFY_MASK != 0 {
                match focusmute_lib::direct::solo_direct_enabled(device) {
                    Ok(current) => {
                        if let Some(action) = gesture.changed(
                            now,
                            current,
                            Duration::from_millis(window_ms.clamp(200, 600)),
                        ) {
                            apply_direct_action(device, monitor, muted, gesture, action, now);
                        }
                    }
                    Err(e) => log::warn!("[direct] could not read Direct state: {e}"),
                }
            }
        }
        Err(e) if e.to_string().contains("timed out") => {}
        Err(e) => log::debug!("[direct] notification poll unavailable: {e}"),
    }
    if let Some(action) = gesture.tick(now) {
        apply_direct_action(device, monitor, muted, gesture, action, now);
    }
}

fn apply_direct_action<M: MuteMonitor>(
    device: &impl ScarlettDevice,
    monitor: Option<&M>,
    muted: bool,
    gesture: &mut DirectGesture,
    ...
    let desired_direct = match action {
        DirectAction::Single { baseline } => {
            log::info!("[direct] single gesture → toggle mute");
            baseline
        }
        DirectAction::Double { baseline } => {
            log::info!("[direct] double gesture → toggle Direct");
            !baseline
        }
    };
    if let Err(e) = focusmute_lib::direct::set_solo_direct_enabled(device, desired_direct) {
        log::warn!("[direct] could not set Direct state: {e}");
        return;
    }
    gesture.suppress_own_notifications(now, desired_direct);
    if matches!(action, DirectAction::Single { .. })
        && let Some(monitor) = monitor
        && let Err(e) = monitor.set_muted(!muted)
    {
        log::warn!("[direct] could not toggle mute: {e}");
    }
}
...
pub fn run_core<P: PlatformAdapter>() -> focusmute_lib::error::Result<()> {
    ...
    let (menu, mut tray_menu) = state::build_tray_menu(&state.config, initial_muted);
    ...
    let mut direct_gesture = DirectGesture::new();
    ...
    loop {
            ...
            && let Some(new_dev) = state.try_reconnect()
        {
            ...
            if state.indicator.is_muted() {
                tray_menu
                    .status_item
                    .set_text(crate::i18n::tr(&state.config.system.language, "muted"));
            }
        }

        // Direct is an opt-in Solo gesture. It deliberately runs before the
        // audio-message drain so a generated WASAPI callback is processed by
        // the normal mute state machine on the following pass.
        if let Some(ref dev) = device {
            handle_direct_button(
                dev,
                main_monitor.as_deref(),
                state.indicator.is_muted(),
                state.config.hardware_button.direct_button_enabled,
                state.config.hardware_button.direct_double_click_ms,
                &mut direct_gesture,
            );
        }
        ...
        if let Some(ref dev) = device
            && let Err(error) = state.indicator.animate(dev, Instant::now())
        {
            log::debug!("[led] mute blink failed: {error}");
        }
        ...
        while let Ok(event) = menu_rx.try_recv() {
            ...
            let settings_toggle_callback = main_monitor.as_ref().map(|monitor| {
                let monitor = Arc::clone(monitor);
                Arc::new(move || {
                    let target_muted = !monitor.is_muted();
                    if let Err(error) = monitor.set_muted(target_muted) {
                        log::warn!(
                            "[hotkey] failed to toggle mute while settings are open: {error}"
                        );
                    }
                }) as crate::settings_dialog::SettingsToggleCallback
            });
...
fn apply_blink_action(
    ...
    let result = match action {
        ...
        super::blink::LedAction::Off => {
            focusmute_lib::led::blank_mute_indicator(device, state.indicator.strategy())
        }
    };
    if let Err(e) = result {
